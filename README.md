# Liline eo Alquimista
Tradução PT-BR de jogo RPG Maker / Visual Novel. Projeto sem fins lucrativos.
# Tradução PT-BR – [Liline eo Alquimista]

Tradução não oficial para português brasileiro.

## Informações
- Jogo original: [Liline and the Alchemist
]
- Engine: RPG Maker / VN
- Idioma original: Japonês
- Idioma da tradução: PT-BR

## Status
🟡 Em andamento / 🟢 Concluído

## Créditos
Tradução e revisão: RobbieSenseiPT-BR

## Aviso
Este projeto não possui fins lucrativos.
Todos os direitos pertencem aos criadores originais.
